...
const PLAYER = chalk.blue('*');                                                   //  declaring * as Player and apply chalk blue colour
...
const OUT_BOUND = chalk.bgRed("Whoopsie! You drop out of the field!");             // Drop out of the field with chalk bgRed (background Colour Red)        
...
const DIRECTION = "Which direction, up(w), down(s), left(a) or right(d)?";         // Keyboard Direction
const QUIT = "Press q or Q to quit the game.";                                     // Keyboard to end the game
const END_GAME = chalk.yellow("Thank you for playing.");                           // Ended the game
const NOT_RECOGNISED =  chalk.bgMagenta("Wrong Direction Key.");                   // Input not recognized
...
class Field {
    ...
    static getDifficulty(){
        console.log(chalk.cyan.bold("Select lor"));                  
        console.log(chalk.green("1: Normal Mode"));                        // Normal Mode
        console.log(chalk.red("2: Hard Mode" ));                           // Hard Mode
        ...
        switch(userInput){
            case "1":
                console.log(chalk.green("Normal Mode Selected!"));
                fieldSize =20;
                break;
            case "2":
                console.log(chalk.yellow("Hard Mode Selected!"));
                fieldSize =40;
                break;
            case "q":
                console.log(chalk.magenta("Are you really gonna quit? Please don't."));
    ...
    updateGame() {                                                      // Update the game
        ...
        do {
            console.log(DIRECTION + " " + QUIT);                        // Request user input
            userInput = prompt();                                       //  Get the user's input

            switch (userInput.toLowerCase()) {
                case "w":
                    console.log(chalk.bgMagenta("Moved Up!"));          // Display user selected input "w"          
                    this.updatePlayer(userInput.toLowerCase());
                    break;
                case "s":
                    console.log(chalk.bgMagenta("Moved Down!"));        // Display user selected input "s"   
                    this.updatePlayer(userInput.toLowerCase());
                    break;
                case "a":
                    console.log(chalk.bgMagenta("Moved Left!"));        // Display user selected input "a" 
                    this.updatePlayer(userInput.toLowerCase());
                    break;
                case "d":
                    console.log(chalk.bgMagenta("Moved Right!"));       // Display user selected input "d" 
                    this.updatePlayer(userInput.toLowerCase());
                    break;
                case "q":
                    console.log(chalk.bgMagenta("End Game!"));          // Display user selected input "q" 
                    this.endGame();
                    break;
                default:
...
const difficultyLevel = fieldSize / 12; // 1 for normal, 2 for hard
